import Middleware = ExpressController.Middleware;
...
export class HttpService
{
    ...
    {
        return ExpressController.get(path, middleware);
    }
    public put(path: string = '*', middleware: Middleware[] = [])
    {
        return ExpressController.put(path, middleware);
    }
    public post(path: string = '*', middleware: Middleware[] = [])
    {
        return ExpressController.post(path, middleware);
    }
    public delete(path: string = '*', middleware: Middleware[] = [])
    {
        return ExpressController.del(path, middleware);
    }
    public options(path: string = '*', middleware: Middleware[] = [])
    {
        return ExpressController.options(path, middleware);
    }
    public patch(path: string = '*', middleware: Middleware[] = [])
    {
        return ExpressController.patch(path, middleware);
    }
    public head(path: string = '*', middleware: Middleware[] = [])
    {
        return ExpressController.head(path, middleware);
    }
    public all(path: string = '*', middleware: Middleware[] = [])
    {
        return ExpressController.all(path, middleware);
    }
    public use(path: string = '*')
    {
        return ExpressController.use(path);
    }
    public baseRoute(path: string = '*')
    {
        return ExpressController.basePath(path);
    }
    public register(router: Express.Router, target: Object)
    {
        return ExpressController.register(router, target);
    }
}

namespace ExpressController {
